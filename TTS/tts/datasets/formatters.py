...
def vivoice(root_path, meta_file, **kwargs):
    """Normalizes the ViVoice meta data file to TTS format using pandas
    ...
    Returns:
        List[dict]: List of dictionaries containing audio paths and corresponding text
    """
    ...
    df['audio_file'] = df['wav_filename'].apply(lambda x: os.path.join(root_path, "wavs", x))
    
    # Clean speaker names (remove @ symbol)
    df['speaker_name'] = df['channel'].str.replace('@', '')
    
    # Convert DataFrame to list of dictionaries with desired format
    items = []
